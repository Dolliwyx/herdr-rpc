...
const presence = (...args: Parameters<typeof presenceFromSnapshot>) => {
  const value = presenceFromSnapshot(...args);
  assert.ok(value);
  return value;
};
...
test('renders default templates with counts, version, and a focused harness icon', () => {
  assert.deepEqual(presence(
    snapshot([{ pane_id: 'pane', agent_status: 'working' }, { pane_id: 'other', agent_status: 'idle' }]), [], DEFAULT_TEMPLATES, 'main', undefined,
...
test('uses private substitutions and no-branch fallback', () => {
  assert.equal(presence(snapshot([{ pane_id: 'pane', agent_status: 'idle' }]), ['client*']).details, 'In Private workspace (Private branch)');
  assert.equal(presence(snapshot([{ pane_id: 'pane', agent_status: 'idle' }])).details, 'In Client Work (No branch)');
});
...
test('renders detached branches and unknown harnesses in custom large-image templates without an icon', () => {
  ...
  const value = presence(
    snapshot([{ pane_id: 'pane', agent_status: 'idle' }], { panes: [{ pane_id: 'pane', cwd: '/repo', agent: 'custom' }] }), [], templates, '@abc123', undefined,
    { largeImageKey: 'herdr', showHarnessIcon: true },
  );
...
test('renders custom small-image templates with existing placeholders', () => {
  const value = presence(
    snapshot([{ pane_id: 'pane', agent_status: 'working' }]), [], { ...DEFAULT_TEMPLATES, smallImageText: '{workspace} {branch} {working}/{detected} {herdrVersion?} {harness?}' }, 'main', undefined,
    { largeImageKey: 'herdr' },
  );
...
test('omits empty rendered fields and truncates by grapheme cluster', () => {
  const value = presence(snapshot([{ pane_id: 'pane', agent_status: 'idle' }]), [], { ...DEFAULT_TEMPLATES, details: '', state: '', largeImageText: '', smallImageText: '' });
...
test('omits small-image fields without a supported focused harness and retains a key with empty text', () => {
  const noFocusedPane = presence(snapshot([{ pane_id: 'pane', agent_status: 'idle' }], { focused_pane_id: 'missing' }), [], DEFAULT_TEMPLATES, undefined, undefined, { largeImageKey: 'herdr' });
  assert.equal('smallImageKey' in noFocusedPane, false);
  const emptyText = presence(
    snapshot([{ pane_id: 'pane', agent_status: 'idle' }]), [], { ...DEFAULT_TEMPLATES, smallImageText: '' }, undefined, undefined,
    { largeImageKey: 'herdr' },
  );
  assert.equal(emptyText.smallImageKey, 'pi');
  assert.equal('smallImageText' in emptyText, false);
  const hidden = presence(snapshot([{ pane_id: 'pane', agent_status: 'idle' }]), [], DEFAULT_TEMPLATES, undefined, undefined, { largeImageKey: 'herdr', showHarnessIcon: false });
