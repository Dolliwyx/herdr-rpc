...
type Call = { command: string; args: string[]; options: { cwd?: string; timeout: number } };
const tick = () => new Promise<void>((resolve) => setImmediate(resolve));

function fakeTimers(): TimerApi {
  return { setTimeout: (callback, delay) => ({ callback, delay }), clearTimeout() {} };
}

test('checks focused public cwd immediately and retains branch on failure', async (t) => {
  const calls: Call[] = []; let changed = 0;
  ...
  resolver.execFile = (_command, _args, _options, callback) => callback(new Error('gone'), '', '');
  ...
  const resolver = new BranchResolver(() => {}, {
    execFile(_command, _args, _options, callback) {
      if (mode === 'main') callback(null, 'main\n', '');
        ...
        const error: ExecError = Object.assign(new Error('not git'), { code: 128 });
...
test('discards stale Git results and immediately checks the new cwd', async (t) => {
  const callbacks: { cwd?: string; callback: ExecCallback }[] = [];
  const resolver = new BranchResolver(() => {}, {
    execFile(_command, _args, options, callback) { callbacks.push({ cwd: options.cwd, callback }); },
  });
  t.after(() => resolver.stop());
...
test('normalizes the one-time Herdr version fallback', () => {
  ...
  const resolver = new VersionResolver(() => { changed += 1; }, {
    execFile(_command, _args, _options, callback) { callback(null, 'herdr 0.7.5\n', ''); },
  });
...
test('uses detached hash and skips private workspaces', async (t) => {
  ...
  const resolver = new BranchResolver(() => { count += 1; }, {
    execFile(_command, args, _options, callback) { calls.push(args); callback(args[0] === 'symbolic-ref' ? new Error('detached') : null, 'abc123\n', ''); },
  });
  t.after(() => resolver.stop());
  resolver.update({ cwd: '/repo', privateWorkspace: false, active: true });
